...
*   **Hesaplama Formülü:** 
    $$\text{Aktif Hub} = \text{Seçilen Hub Kodu (Varsayılan: SAW)}$$
...
*   **Hesaplama Formülü:**
    $$\text{Şablon Sayısı} = \text{Count}(\text{templateId}) \quad \text{where} \quad \text{hubCode} = \text{activeHub}$$
...
*   **Hesaplama Formülü:**
    $$\text{Ortalama Hedef Ground Time} = \frac{\sum_{i=1}^{N} \text{targetMinutes}_i}{N}$$
    *(N: İlgili hub'daki toplam şablon sayısı)*
*   **Örnek:** Şablon hedefleri 40 dk, 45 dk, 60 dk olan bir meydanda: $(40+45+60)/3 = 48.33$ dakika.
*   **Frontend Gösterim Notu:** Başlık altı performans metriklerinde dakika birimi ile özetlenir.
...
*   **Hesaplama Formülü:**
    $$\text{Toplam Uçuş} = \text{Count}(\text{flightId}) \quad \text{where} \quad \text{templateId} = \text{targetTemplateId}$$
...
*   **Hesaplama Formülü:**
    $$\text{SLA OK} = \text{Count}(\text{flightId}) \quad \text{where} \quad \text{actualGroundTimeMinutes} \le \text{targetGroundTimeMinutes}$$
...
*   **Hesaplama Formülü:**
    $$\text{SLA Dışı} = \text{Count}(\text{flightId}) \quad \text{where} \quad \text{actualGroundTimeMinutes} > \text{targetGroundTimeMinutes}$$
...
*   **Hesaplama Formülü:**
    $$\text{SLA Başarı Oranı (\%)} = \left( \frac{\text{SLA OK}}{\text{Toplam Uçuş}} \right) \times 100$$
*   **Örnek:** 34 uçuştan 25'i SLA OK ise: $(25 / 34) \times 100 = \%73.5$.
*   **Frontend Gösterim Notu:** Kartların sağında daire biçimli SVG halkası (Circular progress bar) içinde dinamik olarak gösterilir.
...
*   **Hesaplama Formülü:**
    $$\text{Ortalama Süre} = \frac{\sum_{j=1}^{M} \text{actualGroundTimeMinutes}_j}{M}$$
    *(M: İlgili şablondaki toplam uçuş adedi)*
...
*   **Hesaplama Formülü:**
    $$\text{Sapma (Dakika)} = \text{Ortalama Süre} - \text{Hedef Süre}$$
...
*   **Hesaplama Mantığı:**
    *   **Yeşil (Başarılı):** SLA Başarı Oranı $\ge \%85$ VEYA Sapma $\le 0$ dakika.
    *   **Turuncu (Riskli):** $\%70 \le$ SLA Başarı Oranı $< \%85$ VEYA Sapma $> 0$ ve Sapma $\le 3$ dakika.
...
$$\text{Slack} = \text{Geç Başlangıç Limit} - \text{Planlanan Başlangıç}$$

*   **$\text{Slack} = 0 \text{ dakika}$** olan tüm adımlar doğrudan **Kritik Yol (Critical Path)** üzerindedir. Bu adımlarda meydana gelecek en ufak bir (1) saniyelik gecikme, uçağın pushback saatini doğrudan geciktirir ve rampa rötarına yol açar.
*   **$\text{Slack} > 0 \text{ dakika}$** olan paralel rampa adımlarında (Catering, Cleaning vb.) ise tolerans süresi kadar gecikme yaşanması, uçağın genel kalkışını etkilemez. Ancak gecikme slack süresini aşarsa, bu adım da kritik yola dönüşür.
...
## 7. Database Tasarım Önerisi

İlişkisel veritabanı (PostgreSQL veya Cloud SQL) üzerinde kurulacak şema tasarımı için önerilen tablo Yapıları (DDL) aşağıdadır:

```sql
-- 1. HUB TABLOSU
CREATE TABLE HUB (
    hub_code VARCHAR(3) PRIMARY KEY,
...
-- 2. GROUND TIME TEMPLATE TABLOSU
CREATE TABLE GROUND_TIME_TEMPLATE (
    template_id VARCHAR(50) PRIMARY KEY,
    hub_code VARCHAR(3) REFERENCES HUB(hub_code) ON DELETE CASCADE,
    template_name VARCHAR(100) NOT NULL,
    aircraft_type_condition VARCHAR(30) NOT NULL,
    flight_type_condition VARCHAR(20) NOT NULL,
...
-- 3. PLANLANAN RAMPA ADIMLARI
CREATE TABLE GROUND_TIME_STEP (
    step_id VARCHAR(30) PRIMARY KEY,
    step_name VARCHAR(100) NOT NULL,
    description TEXT
);

-- 4. ADIM BAĞIMLILIK İLİŞKİLERİ (TEMPLATE BAZLI)
CREATE TABLE TEMPLATE_STEP_DEPENDENCY (
    dependency_id SERIAL PRIMARY KEY,
    template_id VARCHAR(50) REFERENCES GROUND_TIME_TEMPLATE(template_id) ON DELETE CASCADE,
    from_step_id VARCHAR(30) REFERENCES GROUND_TIME_STEP(step_id),
    to_step_id VARCHAR(30) REFERENCES GROUND_TIME_STEP(step_id),
    dependency_type VARCHAR(10) DEFAULT 'FS', -- FS, SS, FF
    slack_minutes INTEGER DEFAULT 0,
    is_critical_path BOOLEAN DEFAULT FALSE
);

-- 5. UÇUŞ HAREKET TABLOSU
CREATE TABLE FLIGHT_OPERATION (
    ...
    hub_code VARCHAR(3) REFERENCES HUB(hub_code),
    template_id VARCHAR(50) REFERENCES GROUND_TIME_TEMPLATE(template_id)
);

-- 6. FIİLİ RAMPA GERÇEKLEŞME PAYLAŞIMI
CREATE TABLE FLIGHT_EVENT (
    event_id SERIAL PRIMARY KEY,
    flight_id VARCHAR(50) REFERENCES FLIGHT_OPERATION(flight_id) ON DELETE CASCADE,
    step_id VARCHAR(30) REFERENCES GROUND_TIME_STEP(step_id),
    actual_start_time TIMESTAMP,
    actual_end_time TIMESTAMP,
    status VARCHAR(20) DEFAULT 'NOT_STARTED', -- NOT_STARTED, IN_PROGRESS, COMPLETED, DELAYED
    CONSTRAINT unique_flight_step UNIQUE(flight_id, step_id)
);
```
